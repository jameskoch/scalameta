...
//   of this writing the latest package is `h`, so the next package should be `i`.
...
//  )
  ...
  targeted(
    """package h
      |object ao {
      |  def foo(a: Int = 1, b: Int = 2, c: Int = 3): Unit = ()
      |  def use: Unit = <<foo>>(1, c = 3)
      |}
    """.stripMargin,
    (doc, fooInvocation) => {
      assert(fooInvocation == "h/ao.foo().")
    }
  )
}
